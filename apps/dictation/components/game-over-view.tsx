...
export function GameOverView({
    ...
    const incrementPlayCount = useMutation(api.dictation.incrementPlayCount);
    ...
    useEffect(() => {
        if (!gameId) return;
        try {
            incrementPlayCount({ dictationId: gameId as Id<"dictation_games"> });
        } catch (e) {
    ...
    return (
        <div className="max-w-md mx-auto text-center p-4 sm:p-6 md:p-8 bg-white rounded-xl shadow-2xl">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 sm:mb-6 text-indigo-600">{t?.gameOver || "Game Over"}</h2>
